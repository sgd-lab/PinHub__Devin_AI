...
const ERROR_PATTERNS: Array<{
  ...
  { pattern: /context.*length|token.*limit|too.*long|exceeded/i, type: "model", suggestion: "The request exceeded the model's token limit. Try reducing your prompt length or max tokens setting." },
...
export function parseAIError(error: unknown, provider: string): AIErrorInfo {
  ...
  for (const { pattern, type, suggestion } of ERROR_PATTERNS) {
    if (pattern.test(message)) {
      return {
        ...
        errorType: type,
        suggestion,
        fixPath: type === "auth" || type === "quota" ? "/settings/api-keys" : undefined,
      };
    }
  }
  ...
  return {
    ...
    statusCode,
    errorType: "unknown",
    suggestion: "An unexpected error occurred. Try again or check Settings → API Keys.",
